...
def split(data_frame:DataFrame)->set:
    ...
    for i, row in data_frame.iterrows():
        ...
        for target in target_list:
            data_set.add((row['source'],row['relation'],target.strip()))
...
new_relation_set = split(relations_df)
to_disk('data/relation.csv', ['source', 'relation', 'target'], list(new_relation_set))
print('清洗数据完成')
